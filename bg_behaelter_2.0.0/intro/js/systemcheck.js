...
//   Output: var warning_message:String; true/false
// erstellt: 11.03.09
// geändert: 17.04.09
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
function checkFlash() {
	...
	if (DetectFlashVer(requiredMajorVersion, requiredMinorVersion, requiredRevision)) {
		...
		warning_message = "Version "+versionStr+" ist installiert!";
		return true;
	}
	else {
		...
		if (versionStr != -1) {
			...
			warning_message = 'Sie haben <strong class="warning_red">Flashplayer '+tempString+'</strong> installiert. '+
							'Sie ben&ouml;tigen jedoch mindestens Version '+
							requiredMajorVersion+'.'+requiredMinorVersion+'.'+requiredRevision+'.0! '+
							'Bitte laden Sie sich den aktuellen Flashplayer herunter.<br />'+
							'<div class="flashplayer">'+
							'<a target="_blank" class="quer" href="http://get.adobe.com/de/flashplayer/?promoid=DAFZC"><img alt="FlashPlayer" src="./img/get_adobe_flash_player.png"></a></div>'+
							'<div  style="clear:left"></div>';
			return false;
		}
		// es wurde keine Flash Version gefunden
		else {
			warning_message = '<div class="textblock">Sie haben <strong class="warning_red">keinen Flashplayer</strong> installiert. '+
							//'Bitte laden Sie sich den aktuellen Flashplayer herunter.<br />'+
							//'Das Lernprogramm kann leider nicht gestartet werden, da der Player u.a. für Videos und Interaktionen benötigt wird. Mit einem Klick auf das unten stehende Bild gelangen Sie auf die Internetseiten von Adobe, wo Sie das aktuelle Flash Player Plugin herunterladen und installieren können.'+
							'Das Programm kann leider nicht gestartet werden, da der Player u.a. f&uuml;r Videos und Interaktionen ben&ouml;tigt wird. Mit einem Klick auf das unten stehende Bild gelangen Sie auf die Internetseiten von Adobe, wo Sie das aktuelle Flash Player Plugin herunterladen und installieren k&ouml;nnen.</div>'+
							'<div class="flashplayer">'+
							'<a target="_blank" class="quer" href="http://get.adobe.com/de/flashplayer/?promoid=DAFZC"><img alt="FlashPlayer" src="./img/get_adobe_flash_player.png"></a></div>'+
							'<div  style="clear:left"></div>';
			return false;
...
//	for( i in window ){ str = '"'+i+'"'+" :: "+window[i]; console.log( str ); }
	//console.log( str );
	
*/
	if( top.document ){
		if ( top.document.getElementById("Inhalt") && top.document.getElementById("blindFrame")) {
			return 1;		// die erforderlichen Frames existieren
		} else {
			return 0;		// Frameset wurde nicht korrekt geladen
		}
	} else {
		warning_message =	'<div class="textblock">Der Browser verf&uuml;gt <strong class="warning_red">nicht &uuml;ber ausreichende'+
							'Berechtigungen!</strong><br>'+
							'Das Programm kann leider nicht gestartet werden, da nicht alle Javascript-Funktionen zur Verf&uuml;gung stehen. Diese Einschr&auml;nkung besteht z.B. bei Chrome im Offlinemodus. Verwenden Sie bitte einen anderen Browser oder starten Sie das Programm im Internet.</div>';
		return -1;		// top existiert nicht (z.B. Chrome offline)
	}
